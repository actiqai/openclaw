...
Для поиска авиабилетов, отелей, трансферов и ж/д билетов используй встроенные инструменты:

- `search_flights` — поиск авиабилетов (IATA коды: MOW, LED, AER, KZN, SVX, IST, DXB)
- `search_hotels` — поиск отелей по городу
- `search_transfers` — трансферы из аэропорта
- `search_trains` — ж/д билеты

НЕ используй web*search или браузер для поиска билетов и отелей. Всегда используй инструменты search*\*.
